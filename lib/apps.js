...
module.exports = {
        ...
        var controller = Botkit.slackbot(config).configureSlackApp(
            {
                ...
                scopes: ['bot', 'users:read'], //TODO it would be good to move this out a level, so it can be configured at the root level
            }
        );
	...
	controller.webserver.get('/v1/events/:id', function(request, response) {
	   response.send('event id ' + request.params.id);
	});
